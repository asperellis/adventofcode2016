...
// arr that holds all the bots and the outpus
let bots = [];
let outputs = [];

// Tried to use classes this time around.
// First attempt was to have an id, high, and low values and then give and take methods
// fails to account for triggering commands ONLY when bot has two vals
// use resource https://raw.githubusercontent.com/tlareg/advent-of-code/master/src/2016/day10/solution.js for solution
// more of a learning attempt than self solved but some code is mine.
// just wanted to give credit where credit is due. while also acknowledging I didnt copy paste
// i got stuck on fig out how to run gives with a bot that didnt have any. overthunk it and assumed i would need to track vs just holding until a bot has 2 vals

// THINGS LEARNED TODAY
// [...this.vals, val] - to add a var to array - didnt know this syntax
// first attempt at using classes. starting to make sense!
// state seems like a think i should look into more. mostly wrote my find functs outside in global
// great practice in structuring code. feeling good my idea to use classes wasn't a bad idea. just need more exp with using them

// class for outputs - all they do is take vals and get added to outputs arr
// this class was taken from above example. ignored state because I had state functions written already in global
class Output {
  constructor(id) {
    this.id = id;
    this.vals = [];
...
// bot class - has id, vals for low/high and commands
// this class was taken from above example. ignored state because I had state functions written already in global
class Bot {
  constructor(id) {
    this.id = id;
  ...
  // add a value to the arr/if has two vals and command, run
  addVal(val) {
    this.vals.push(val);
    if (this.vals.length > 1 && this.commands.length > 0) {
      this.executeCommand();
    }
  }
  // add command to arr/if has two vals and command, run
  addCommand(command) {
  ...
  executeCommand() {
    ...
    const [lowValTarget, lowValTargetId, highValTarget, highValTargetId] = this.commands.pop();
    // run the command and transfer vals to other bots or outputs
    findById(lowValTarget, lowValTargetId).addVal(lowVal);
    findById(highValTarget, highValTargetId).addVal(highVal);
  }
};

// i had a create bot and find bot fun but expanded to outputs and id find once I sought help
const createBot = function(id) {
  var newBot = new Bot(id);
  bots.push(newBot);
  return newBot;
}

const createOutput = function(id) {
  const newOutput = new Output(id)
  outputs.push(newOutput);
  return newOutput
}

const findBot = function(id) {
  return bots.find(bot => bot.id === id) || createBot(id);
}

const findOutput = function(id) {
  return outputs.find(output => output.id === id) || createOutput(id);
}

const findById = function (target, targetId) {
  if (target === 'bot') {
    return findBot(targetId);
  }
  if (target === 'output') {
    return findOutput(targetId);
  }
  return null;
}

// all mine - at first I ran out of order thinking vals needed to be added first but that wasnt the case
// instructions were lame on this one.
// for each input line - check if value command or give commmand and execute
input.forEach(function(instruction) {
  if(instruction.indexOf("value") >= 0){
    instruction = instruction.replace("value ", "").split(" goes to bot ");
    const bot = findBot(parseInt(instruction[1])); // find bot to add val, if there isnt one make it.
    bot.addVal(parseInt(instruction[0])); // add val to bot
  } else {
    instruction = instruction.replace("bot ", "").replace(" gives low to ",",").replace(" and high to ",",").split(',');
    botId = parseInt(instruction[0]);
    giveLowType = instruction[1].split(' ')[0];
    giveLowId = parseInt(instruction[1].split(' ')[1]);
    giveHighType = instruction[2].split(' ')[0];
    giveHighId = parseInt(instruction[2].split(' ')[1]);
    const bot = findBot(botId); // find bot to add command if there isnt one make it
    bot.addCommand([giveLowType, giveLowId, giveHighType, giveHighId]); // add command
  }
});
// mine - multiply the vals of first three indexes of output - had to find because they are out of order.
console.log("Part 2 Answer: ", findOutput(0).vals[0] * findOutput(1).vals[0] * findOutput(2).vals[0])
