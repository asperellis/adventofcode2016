...
// function that takes room string and key and returns if its a real room or not
const roomIsReal = function(str, key) {
  // object of char counts and array for sorting
  let charCounts = {};
  let sorted = [];
  let keyCheck = "";

  // for each char add to object to keep track of count
  for(var i = 0; i < str.length; i++)
  {
      let objKey = str[i];
      if(!charCounts[objKey]){
       charCounts[objKey] = 0;
      }
      charCounts[objKey]++;
  }

  // for each prop add to array
  for (let character in charCounts)
    sorted.push([character, charCounts[character]]);

  // sort asc then reverse - need to read more about sort - should do by that alone
  sorted.sort(function(a, b) {
    let c1 = a[0].toLowerCase();
    let c2 = b[0].toLowerCase();
  ...
  sorted.slice(0,5).map(function(char){
    keyCheck += char[0];
  });
...
// function that decrypts a string into room name
const decryptRoomName = function(name, shift) {
  // string to hold decrypted string
  let roomName = "";
  // loop through string
  for(let j = 0; j < name.length; j++){
    ...
    let c = name[j];
		// If it's a letter...
		if (c.match(/[a-z]/i)) {
			// Get its code - unicode for char
			let code = name.charCodeAt(j);
      // set c = to new char after shift 97 is start of lowercose letters (a)
      // so sets at 0-26 then adds shift and mods 26 then adds back 97 to get new char from shifted code
...
// go through each data line and check if its real if it is add to idSum
data.forEach(function(str){
  let arr = str.split('-');
  let idAndKey = arr.pop();
  // gets five chars inside the []
  let key = idAndKey.substr(idAndKey.indexOf('[') + 1, 5);
  let id = parseInt(idAndKey.substr(0, idAndKey.indexOf('[')));
  let stringToCheck = arr.join().replace(/,/g, "");
  let stringToDecrypt = arr.join().replace(/,/g, " ");
  // add to id count
  if(roomIsReal(stringToCheck, key))
    idSum+=id;
    // decrypt room name
    let roomName = decryptRoomName(stringToDecrypt, id);
    if(roomName.indexOf("northpole") > -1)
      console.log("Part 2: North Pole Objects Location: ", id);
});

console.log("Part 1: Sum of IDs: ", idSum);
