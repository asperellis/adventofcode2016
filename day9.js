...
while (i !== -1) {
  if(data[i] === '('){
    let marker = data.substr(i,data.indexOf(')')+1);
    let markerData = marker.replace(')','').replace('(','');
    markerData = markerData.split('x');
    let charsToRepeat = markerData[0];
    let repeatTimes = markerData[1];
    let stringToRepeat = data.substr(marker.length, charsToRepeat);

    for(let k = 0; k < repeatTimes; k++) {
      decryptedData+=stringToRepeat;
    }

    data = data.replace(marker+stringToRepeat,'');
  }

  i+=data.indexOf('(');
}

// tired to regex all the markers out and do it mathematically but i dont think its possible to do that :9
const allMarkers = data.match(/(\((\d+)x(\d+)\)+(?:\((\d+)x(\d+)\)+)*)/g);
let final = [];
let row = [];
let tracker = 0;
let extraChars = 0;
...
allMarkers.forEach(function(str){
    ...
    if(start) {
      start = false;
      tracker = parseInt(arr[0].length) + 2 + parseInt(arr[0].split('x')[0]);
      if(data.indexOf(arr[0]) > 0)
        extraChars += data.indexOf(arr[0])-1; // get early chars
    }

    if(data.indexOf(arr[0]) <= tracker && tracker > data.indexOf(arr[0]) + 2 + arr[0].length) {
      row.push(arr);
    } else {
       final.push(row);
       row = [];
       row.push(arr);
       extraChars += data.indexOf(arr[0]) - 1 - tracker;
       tracker += parseInt(arr[0].length) + 2 + parseInt(arr[0].split('x')[0]);
    }
});

extraChars += data.length - 1 - tracker;
final.push(row);

final.forEach(function(arr){
  var merged = [].concat.apply([], arr);
  var multiplier = parseInt(merged[0].split('x')[1]);

  for(var i = 0; i < arr.length; i++){
    if(arr[i].length > 1) {
       var innerMerged = [].concat.apply([], arr[0]);
       var innerSum = 0;
       for(var k = innerMerged.length - 1; k >= 0; k--){
         var numbers = innerMerged[k].split('x');
         if(k === innerMerged.length - 1) {
           innerSum += parseInt(numbers[0]) * parseInt(numbers[1]);
         } else {
           innerSum *= parseInt(numbers[1]);
         }
       }
       if(i !== 0)
        innerSum *= multiplier;
...
console.log('Part 1: Decrypted Data Length: ', decryptedData.length); // first part solved on my own
console.log('Part 2: Decrypted Data Length: ', sum); // 3033225360 is waaaaay to hight
console.log(unzipLen(data2, true)); // below answer found with help on reddit
